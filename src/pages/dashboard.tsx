...
function ProjectRow({ project, index }: { project: Project; index: number }) {
  ...
  return <Link href={`/project/${project.id}`} data-testid={`link-project-${project.id}`} className={`fade-up stagger-${Math.min(index + 1, 5)} group grid grid-cols-[minmax(180px,1.2fr)_105px_95px_110px_90px_90px_105px_24px] items-center gap-3 border-b border-border/70 px-5 py-3.5 last:border-0 hover:bg-[#fcf5e5]`}>
...
export default function Dashboard() {
  ...
  const upcoming = useMemo(
    () =>
      [...projects]
        .filter((project) => Boolean(project.nextMilestoneDate))
        .sort((a, b) => (a.nextMilestoneDate || '').localeCompare(b.nextMilestoneDate || ''))
        .slice(0, 4),
    [projects],
  );
  // "My projects" is keyed off the signed-in user's stable id (project.leadId),
  // never a display-name string comparison.
  const myProjectCount = useMemo(() => projects.filter((project) => project.leadId === user?.id).length, [projects, user?.id]);

  // Dynamically find project nearest completion (highest progress < 100% or highest progress)
  const nearCompletionProject = useMemo(() => {
    if (!projects.length) return null;
    const candidates = projects.filter((p) => p.progress > 0 && p.progress < 100);
    if (candidates.length === 0) return null;
    return candidates.reduce((highest, curr) => (curr.progress > highest.progress ? curr : highest), candidates[0]);
  }, [projects]);
  ...
  return <div className="mx-auto max-w-[1540px] px-5 pb-14 pt-8 md:px-10 md:pt-10">
    <section className="fade-up flex flex-col justify-between gap-7 lg:flex-row lg:items-end">
      <div>
        ...
        <p className="mt-4 max-w-[570px] text-[13px] leading-6 text-muted-foreground">
          {stats.total === 0
            ? role === 'lead'
              ? 'Your portfolio is clean and ready. Click "New project" to create your first project and begin tracking stages, milestones, and issues.'
              : 'Your portfolio is currently empty. Project leads can add new projects to start tracking delivery and health.'
            : role === 'lead'
              ? `A working view of ${myProjectCount} ${myProjectCount === 1 ? 'project' : 'projects'} you manage, plus the updates that need your attention.`
              : `A composed read on ${stats.total} active projects across Encalm Hospitality. ${atRisk.length} ${atRisk.length === 1 ? 'decision needs' : 'decisions need'} your attention this week.`}
        </p>
      </div>
    ...
    <section className={`mt-8 grid gap-5 ${portfolioExpanded ? 'grid-cols-1' : 'lg:grid-cols-[1.35fr_.65fr]'}`}>
      <div className="min-w-0">
        ...
        <div className={`mt-4 hidden overflow-x-auto rounded-2xl border border-[#e1dccf] bg-[#f7f4ec] text-[#173e49] shadow-sm shadow-[#173e49]/[.03] md:block [&>a]:!bg-[#f7f4ec] [&>a]:!text-[#173e49] [&>a:hover]:!bg-[#fcf5e5] ${portfolioExpanded ? 'max-h-none' : 'max-h-[560px] overflow-y-auto'}`} style={{ backgroundColor: '#f7f4ec' }}>
          <div className="flex items-center justify-between gap-4 border-b border-border bg-[#f7f4ec] px-5 py-2.5">
            ...
            >
              ...
              <span>{portfolioExpanded ? 'Standard view' : 'Expand full width'}</span>
            </button>
          </div>
          <div className="sticky top-0 z-10 grid grid-cols-[minmax(180px,1.2fr)_105px_95px_110px_90px_90px_105px_24px] gap-3 border-b border-border bg-[#f7f4ec] px-5 py-3 font-mono text-[9px] uppercase tracking-[.1em] text-muted-foreground">
            <span>Project</span>
            <span>Status</span>
            <span>Health</span>
            <span>Progress</span>
          ...
          {filteredProjects.length ? filteredProjects.map((project, index) => <ProjectRow key={project.id} project={project} index={index} />) : <div className="p-10 text-center text-[12px] text-muted-foreground">No projects match this view. Try a broader search.</div>}
        </div>
      ...
      <div className={portfolioExpanded ? 'grid gap-5 md:grid-cols-2' : 'space-y-5'}>
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm shadow-[#173e49]/[.03]">
          <div className="flex items-start justify-between">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Next up</p>
              <h2 className="mt-2 text-[18px] font-extrabold tracking-[-.03em]">Upcoming milestones</h2>
            </div>
            ...
            {upcoming.length ? (
              upcoming.map((project) => (
                <Link
                  ...
                  data-testid={`link-milestone-${project.id}`}
                  key={project.id}
                  className="group flex gap-3"
                >
                  <span className="mt-0.5 grid size-8 shrink-0 place-items-center rounded-lg bg-[#f7f4ec] text-[#9a711f]">
                    <Clock3 size={14} />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center justify-between gap-2">
                      <span className="truncate text-[11px] font-bold">{project.nextMilestone}</span>
                      <ArrowUpRight
                        size={13}
                        className="shrink-0 text-muted-foreground/45 group-hover:text-foreground"
                      />
                    </span>
                    <span className="mt-1 block truncate text-[10px] text-muted-foreground">{project.name}</span>
                    <span className="mt-1 block font-mono text-[9px] uppercase tracking-[.1em] text-[#9a711f]">
                      {milestoneWindowLabel(project.nextMilestoneDate)}
                    </span>
                  </span>
                </Link>
              ))
            ) : (
              <p className="text-[11px] leading-5 text-muted-foreground">No upcoming milestones recorded yet.</p>
            )}
          </div>
        </div>
        {nearCompletionProject && (
          <div className="rounded-2xl border border-[#d0e0d9] bg-[#edf5f0] p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[.16em] text-[#2e7c67]">Near completion</p>
                <h2 className="mt-2 text-[18px] font-extrabold tracking-[-.03em] text-[#173e49]">Opening sequence</h2>
              </div>
              <CheckCircle2 size={18} className="text-[#3d9a7e]" />
            </div>
            <p className="mt-4 text-[11px] leading-5 text-[#517167]">
              <strong className="text-foreground">{nearCompletionProject.name}</strong> is at{' '}
              <strong className="text-foreground">{nearCompletionProject.progress}%</strong> complete in stage &ldquo;
              {nearCompletionProject.phases.find((phase) => phase.status === 'active')?.name || 'Delivery'}&rdquo;. Target completion is{' '}
              {nearCompletionProject.targetLabel}.
            </p>
            <div className="mt-5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="grid size-7 place-items-center rounded-full border-2 border-[#edf5f0] bg-[#d6a95d] text-[9px] font-bold text-[#173e49]">
                  {initialsOf(leadName(nearCompletionProject.leadId))}
                </span>
                <span className="text-[10px] font-semibold text-muted-foreground">
                  Lead: {leadName(nearCompletionProject.leadId)}
                </span>
              </div>
              <Link
                href={`/project/${nearCompletionProject.id}`}
                data-testid="link-near-completion"
                className="flex items-center gap-1 text-[11px] font-bold text-[#2e7c67] hover:text-[#173e49]"
              >
